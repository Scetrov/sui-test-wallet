...
1. Enable **Developer mode** via the toggle switch in the top right.
1. Click **Load unpacked** and select the `dist` directory generated by the build step.
...
1. **Create conventional commits:**

   ```bash
   npm run commit
   ```

   _Uses Commitizen with the conventional changelog preset so commit history stays structured._

### Release Workflow

Releases are tag-driven and reuse the exact extension bundle built by CI for the tagged commit.

1. Update the version with npm so `package.json` and `manifest.json` stay aligned:

   ```bash
   npm version patch
   ```

   _Use `minor` or `major` when appropriate._

1. Push the release commit and tag together:

   ```bash
   git push origin main --follow-tags
   ```

1. GitHub Actions waits for the CI run for that commit, downloads the stored artifact, and publishes the GitHub release without rebuilding the extension.

If you push a tag later for an existing commit, the release workflow will reuse the retained CI artifact for that commit as long as it is still available in GitHub Actions artifacts.
